export const PROJECTS = [
  {
    id: 'ai-code-auditor',
    title: 'AI-Powered Code Auditor',
    description: 'An automated security auditing tool for Python projects using LLMs to detect vulnerabilities and suggest fixes.',
    tech: ['Python', 'Gemini API', 'Docker', 'FastAPI'],
    github: 'https://github.com/sudarshan/ai-code-auditor',
    demo: 'https://demo.sudarshan.com/ai-auditor',
    category: 'AI',
    image: 'https://picsum.photos/seed/code-audit/800/600'
  },
  {
    id: 'distributed-crawler',
    title: 'Distributed Web Crawler',
    description: 'A high-performance crawler built to handle millions of pages with a focus on efficiency and polite crawling.',
    tech: ['Go', 'Redis', 'PostgreSQL', 'gRPC'],
    github: 'https://github.com/sudarshan/distributed-crawler',
    demo: 'https://demo.sudarshan.com/crawler',
    category: 'Backend',
    image: 'https://picsum.photos/seed/crawler/800/600'
  },
  {
    id: 'neural-viz',
    title: 'Neural Net Visualizer',
    description: 'An interactive web application developed to visualize the training process of neural networks in real-time.',
    tech: ['TypeScript', 'D3.js', 'React', 'TensorFlow.js'],
    github: 'https://github.com/sudarshan/neural-viz',
    demo: 'https://neural-viz.vercel.app',
    category: 'AI',
    image: 'https://picsum.photos/seed/neural/800/600'
  },
  {
    id: 'blockchain-vote',
    title: 'Decentralized Voting Platform',
    description: 'Ensuring transparency in university elections through a secure blockchain-based voting system.',
    tech: ['Solidity', 'Ethereum', 'Web3.js', 'Next.js'],
    github: 'https://github.com/sudarshan/blockchain-voting',
    demo: 'https://u-vote-blockchain.io',
    category: 'Web',
    image: 'https://picsum.photos/seed/voting/800/600'
  },
  {
    id: 'health-track-pro',
    title: 'HealthTrack Pro',
    description: 'A comprehensive health monitoring dashboard with real-time vitals tracking and predictive health alerts.',
    tech: ['React', 'TypeScript', 'Tailwind CSS', 'Firebase'],
    github: 'https://github.com/sudarshan/health-track',
    demo: 'https://healthtrack-pro.vercel.app',
    category: 'Web',
    image: 'https://picsum.photos/seed/health/800/600'
  },
  {
    id: 'system-monitor',
    title: 'Linux Kernel Module',
    description: 'A character driver for hardware monitoring used to track CPU temperature and fan speeds at the kernel level.',
    tech: ['C', 'Linux Kernel', 'Shell Scripting'],
    github: 'https://github.com/sudarshan/kernel-monitor',
    demo: '#',
    category: 'Backend',
    image: 'https://picsum.photos/seed/linux/800/600'
  }
];

export const SKILLS = {
  languages: [
    { name: 'Python', level: 95 },
    { name: 'JavaScript', level: 90 },
    { name: 'TypeScript', level: 85 },
    { name: 'C++', level: 80 },
    { name: 'Go', level: 75 },
    { name: 'Solidity', level: 65 }
  ],
  technologies: [
    { name: 'React', level: 90 },
    { name: 'Next.js', level: 85 },
    { name: 'FastAPI', level: 80 },
    { name: 'Tailwind CSS', level: 95 },
    { name: 'PostgreSQL', level: 85 },
    { name: 'Redis', level: 80 },
    { name: 'Docker', level: 85 }
  ],
  tools: [
    { name: 'Kubernetes', level: 70 },
    { name: 'AWS/GCP', level: 75 },
    { name: 'Git', level: 95 },
    { name: 'GitHub Actions', level: 85 },
    { name: 'Terraform', level: 60 },
    { name: 'Linux Admin', level: 85 }
  ]
};

export const EXPERIENCE = [
  {
    company: 'Google AI Studio Build',
    role: 'Software Development Intern',
    period: 'Jan 2026 - Present',
    description: 'Working on full-stack application scaffolding and automated agent evaluation systems.',
    points: [
      'Optimized code generation pipelines for 20% faster execution.',
      'Designed and implemented responsive Bento Grid UI templates.',
      'Collaborated on improving semantic search for code libraries.'
    ]
  },
  {
    company: 'University AI Lab',
    role: 'Research Assistant',
    period: 'Aug 2024 - Dec 2025',
    description: 'Conducting research on Large Language Models and their application in automated code refactoring.',
    points: [
      'Co-authored a paper on LLM-based security vulnerability detection.',
      'Managed high-performance computing clusters for model training.',
      'Developed datasets for fine-tuning open-source models.'
    ]
  },
  {
    company: 'Tech Innovators Inc.',
    role: 'Backend Engineering Intern',
    period: 'May 2024 - Aug 2024',
    description: 'Developed cloud-native microservices and optimized database queries for high-traffic applications.',
    points: [
      'Improved API latency by 30% through advanced caching strategies.',
      'Containerized legacy monitoring applications using Docker.',
      'Documented and tested 50+ RESTful API endpoints.'
    ]
  }
];

export const EDUCATION = [
  {
    school: 'Anna University',
    degree: 'B.Tech in Computer Science and Engineering',
    period: '2022 - 2026',
    description: 'GPA: 3.96/4.0. Recipient of Merit Scholarship for Academic Excellence.'
  },
  {
    school: 'International Institute of IT',
    degree: 'Summer Research Fellowship',
    period: 'June 2025 - July 2025',
    description: 'Focused on Advanced Algorithms and Computational Complexity.'
  }
];

export const ACHIEVEMENTS = [
  {
    title: 'Google HashCode Global Finalist',
    issuer: 'Google',
    date: '2025'
  },
  {
    title: 'AWS Certified Solutions Architect',
    issuer: 'Amazon Web Services',
    date: '2025'
  },
  {
    title: 'Winner - City-Wide Smart City Hackathon',
    issuer: 'SmartCity Gov',
    date: '2024'
  },
  {
    title: 'Microsoft Certified: Azure Fundamentals',
    issuer: 'Microsoft',
    date: '2024'
  },
  {
    title: 'Dean\'s List for 6 Consecutive Semesters',
    issuer: 'Anna University',
    date: '2022-2025'
  }
];

